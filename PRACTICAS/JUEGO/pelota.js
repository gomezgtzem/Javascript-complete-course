...
class Bola {
  ...
  colisiones() {
    if (this.posX > 780) this.direccionX = -1;
    if (this.posX < 15) {
      this.posX = 400;
      this.vidas -= 1;
      if (this.vidas < 1) window.location.reload();
    }
    if (this.posY > 500) this.direccionY = -1;
    if (this.posY < 20) this.direccionY = 1;

    if (
      this.posX < jugador.posX + 20 &&
